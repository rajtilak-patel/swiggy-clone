...
const Banner = () => {
  ...
  const nextData = ()=>{
       if(slide === food.length-7) return false   
       setSlide(slide+7)
            ...
            <div className='flex overflow-hidden mt-2'>
                  {
                    food.map((data,index)=>{
                       return (
                       <div style={{transform:`translateX(-${slide*100}%)`}} key={index} className='w-[150px] shrink-0 p-2 duration-[800ms]'>
                         <img src={data.image} alt={data.name} className='rounded-full w-[100px] h-[100px] mx-auto'/>
                         <h1 className='text-center font-semibold'>{(data.name).slice(0,12)}...</h1>
                       </div>)
                    })
                  }
            </div>
